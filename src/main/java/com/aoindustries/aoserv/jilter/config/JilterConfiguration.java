...
public class JilterConfiguration {
    ...
    while (e.hasMoreElements()) {
      ...
      if (key.startsWith(businessesKey + ".")) {
        ...
        if (!domainAddresses.containsKey(domain)) {
          domainAddresses.put(domain, new HashSet<>());
        }
        ...
        if (addresses == null) {
          domainAddresses.put(domain, addresses = new HashSet<>());
        }
  ...
  public void saveIfChanged(String comment) throws IOException {
    ...
    if (!matches) {
      ...
      try {
        ...
        // Email settings
        if (smtpServer != null) {
          props.setProperty("smtp.server", smtpServer);
        }
        if (emailSummaryFrom != null) {
          props.setProperty("email.summary.from", emailSummaryFrom);
        }
        if (emailSummaryTo != null) {
          props.setProperty("email.summary.to", emailSummaryTo);
        }
        if (emailFullFrom != null) {
          props.setProperty("email.full.from", emailFullFrom);
        }
        if (emailFullTo != null) {
          props.setProperty("email.full.to", emailFullTo);
        }
  ...
  public static boolean equalsMapSetString(Map<?, ? extends Set<?>> map1, Map<?, ? extends Set<?>> map2) {
    ...
    if (!equals(keySet1, keySet2)) {
      if (log.isTraceEnabled()) {
        log.trace("equals(Map<?, ? extends Set<?>> map1, Map<?, ? extends Set<?>> map2): keySet1 != keySet2");
      }
      return false;
    }
  ...
  public static boolean equalsMap(Map<?, ?> map1, Map<?, ?> map2) {
    ...
    if (!equals(keySet1, keySet2)) {
      if (log.isTraceEnabled()) {
        log.trace("equals(Map<?, ?> map1, Map<?, ?> map2): keySet1 != keySet2");
      }
      return false;
    }
